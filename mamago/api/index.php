...
// Depuis la fusion, `services` est partage avec le site vitrine : le
// controleur dedie masque les cartes marketing et gere les libelles
// bilingues. Voir ServicesController.
$services = new ServicesController();
$router->get('/services',         fn ($r) => $services->index($r));
$router->get('/services/{id}',    fn ($r, $p) => $services->show($r, $p));
$router->post('/services',        fn ($r) => $services->store($r));
$router->put('/services/{id}',    fn ($r, $p) => $services->update($r, $p));
$router->delete('/services/{id}', fn ($r, $p) => $services->destroy($r, $p));
